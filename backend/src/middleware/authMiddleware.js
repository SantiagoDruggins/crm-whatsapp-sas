...
async function authMiddleware(req, res, next) {
  try {
    ...
    const result = await query(
      `
      SELECT u.id, u.nombre, u.email, u.rol, u.is_active, u.empresa_id, u.crm_role_id,
    ...
    let crmPermisos = user.crm_permisos && typeof user.crm_permisos === 'object' ? user.crm_permisos : {};
    let crmIsFullAccess = !!user.crm_is_full_access;
    if (user.empresa_id && !user.crm_role_id && user.rol === 'admin') {
    ...
    req.user = {
      ...
      email: user.email,
      rol: user.rol,
      empresaId: user.empresa_id || empresaId || null,
      crmRoleId: user.crm_role_id,
      crmPermisos,
      crmIsFullAccess,
    };
